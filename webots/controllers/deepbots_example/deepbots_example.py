...
class CartpoleRobot(RobotSupervisor):
    def __init__(self):
        ...
        self.observation_space = Box(low=np.array([-0.4, -np.inf, -1.3, -np.inf]),
                                     ...
                                     dtype=np.float64)
        #self.observation_space = Dict({"pos":Box(-0.4,0.4,[1]),"vel":Box(-np.inf,np.inf,[1]),"angle":Box(-1.3,1.3,[1]),"pole_vel":Box(-np.inf,np.inf,[1])})
    ...
    def get_observations(self):
        ...
        #return {"pos":cartPosition,"vel":cartVelocity,"angle":poleAngle,"pole_vel":endpointVelocity}
...
env = CartpoleRobot()
agent = PPOAgent(numberOfInputs=env.observation_space.shape[0], numberOfActorOutputs=env.action_space.n, use_cuda=False)
solved = False
episodeCount = 0
episodeLimit = 2000
